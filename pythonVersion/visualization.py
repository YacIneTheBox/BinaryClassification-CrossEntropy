...
def plot_sigmoid_output(model, x_train, y_train, output_file="sigmoid_output.png"):
    ...
    plt.plot(x_range, prob_values, "b-", linewidth=2, label="P(y=1|x) = σ(f(x))")
    plt.scatter(
        x_train,
        y_train,
        c="black",
...
def plot_likelihood_and_nll(
    beta1_range, likelihoods, nlls, output_file="likelihood_nll_variation.png"
):
    """
    ...
    ax1.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Max à β₁={optimal_beta1_lik:.4f}",
    )
    ax1.scatter(
        [optimal_beta1_lik], [likelihoods[max_lik_idx]], c="red", s=150, zorder=5
    )
    ...
    ax2.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Min à β₁={optimal_beta1_nll:.4f}",
    )
    ax2.scatter([optimal_beta1_nll], [nlls[min_nll_idx]], c="green", s=150, zorder=5)
    ax2.set_xlabel("β₁ (biais de sortie)", fontsize=12)
    ax2.set_ylabel("Negative Log-Likelihood", fontsize=12)
    ax2.set_title("Variation du Negative Log-Likelihood en fonction de β₁", fontsize=14)
